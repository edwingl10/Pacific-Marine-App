...
public class rmag_drag : MonoBehaviour, IDragHandler, IBeginDragHandler, IEndDragHandler
{
    ...
    public void OnDrag(PointerEventData eventData)
    {
        ...
        if (rmag_target.body == false)
        {
            neck_target.GetComponent<Animator>().SetBool("targetHelp", true);
        }
        else if (rmag_target.body)
        {
            Instruction_texts.gameObject.GetComponent<Text>().text = "Record your observations of Roma on the clipboard.";
            play_clipboard();
        }
